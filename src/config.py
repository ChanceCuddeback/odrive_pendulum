...
ureg = UnitRegistry()
# Define the ureg units
ureg.define('second = [time] = 1')
ureg.define('millisecond = 1e-3 * second')
ureg.define('gram = 1e-3 * kilogram')
ureg.define('newton = kilogram * meter / second**2')
ureg.define('newton_meter = newton * meter')
ureg.define('radian = [angle] = 1')
...
def set_config(odrv):
    ...
    odrv.axis0.config.motor.torque_constant = 0.0827 # 8.27 / motor kV
    odrv.axis0.config.motor.current_soft_max = 30
    odrv.axis0.config.motor.current_hard_max = 40
    ...
    odrv.axis0.controller.config.input_mode = odrive.enums.InputMode.PASSTHROUGH
    odrv.axis0.controller.config.enable_torque_mode_vel_limit = True
